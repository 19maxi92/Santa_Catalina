<?php
/**
 * Pedidos Online para empleados — clon recortado del panel de admin,
 * filtrado por la sucursal asignada al empleado. Sin config de turnos,
 * sin eliminar (eso queda para admin).
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        switch ($accion) {
            case 'tomar_pedido':
                if ($id) {
                    $pdo->prepare("UPDATE pedidos SET estado = 'Preparando', tomado = 1, updated_at = NOW() WHERE id = ? AND ubicacion = ?")
                        ->execute([$id, $mi_ubicacion]);
            ...
            case 'cambiar_estado':
                $estado = $_POST['estado'] ?? '';
                if ($id && $estado) {
                    $pdo->prepare("UPDATE pedidos SET estado = ?, tomado = 1, updated_at = NOW() WHERE id = ? AND ubicacion = ?")
                        ->execute([$estado, $id, $mi_ubicacion]);
...
// Obtener pedidos online de la sucursal del empleado
$sql = "SELECT * FROM pedidos WHERE observaciones LIKE '%PEDIDO ONLINE%' AND ubicacion = ? AND DATE(created_at) = ?";
$params = [$mi_ubicacion, $fecha];
...
$stmt_stats = $pdo->prepare("SELECT COUNT(*), COALESCE(SUM(precio),0), COUNT(CASE WHEN estado='Pendiente' THEN 1 END) FROM pedidos WHERE observaciones LIKE '%PEDIDO ONLINE%' AND ubicacion = ? AND DATE(created_at) = ?");
$stmt_stats->execute([$mi_ubicacion, $fecha]);
...
<head>
    ...
    <title>Pedidos Online - <?= htmlspecialchars($mi_ubicacion) ?></title>
    ...
    <header class="bg-gradient-to-r from-teal-600 to-cyan-700 text-white shadow-xl">
        <div class="max-w-6xl mx-auto px-4 py-4 flex justify-between items-center">
            <div class="flex items-center gap-4">
                <a href="dashboard.php" class="text-teal-200 hover:text-white transition-colors">
                ...
                <div>
                    ...
                    <p class="text-teal-200 text-sm"><?= htmlspecialchars($mi_ubicacion) ?></p>
                </div>
                <?php if ($pendientes > 0): ?>
            ...
            <span class="text-teal-200 text-sm"><?= htmlspecialchars($_SESSION['empleado_name'] ?? '') ?></span>
        </div>
    </header>
